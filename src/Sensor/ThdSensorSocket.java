...
public class ThdSensorSocket extends Thread implements Runnable, ActionListener
{
	public static final String myClassName = "ThdSensorSocket";
	public volatile Driver driver = new Driver();

	public SensorServerSocket parent = null;
	...
	public static volatile LinkedList<ThdSensorSocket> list_outbound_connections = new LinkedList<ThdSensorSocket>();
	
	/**iterate through this list to send all collected sensor data across*/
	public static volatile LinkedList<ThdSensorSocket> ALL_CONNECTIONS = new LinkedList<ThdSensorSocket>();
	
	Timer tmrUpdate_1_SEC = null;
	
	public ThdSensorSocket(SensorServerSocket par, Socket skt)
	{
		try
		{
			parent = par;
	...
	public boolean send(String out)
	{
		try
		{			
			//omit sending line of own traffic. i.e., do not capture and retransmit the lines of our sensor information to the same client
			if(out != null && out.contains(""+distant_end_port) && out.contains(distant_end_ip))
				return false;
